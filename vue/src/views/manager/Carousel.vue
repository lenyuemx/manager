<template>
  <div class="manager-page-container">
        ...
        <el-table-column label="轮播图" align="center">
          <template v-slot="scope">
            <div class="table-img-wrapper">
              <el-image v-if="scope.row.img" :src="scope.row.img" :preview-src-list="[scope.row.img]" class="carousel-img"></el-image>
            </div>
          </template>
        </el-table-column>
    ...
    <el-dialog title="轮播图信息" :visible.sync="fromVisible" width="500px" :close-on-click-modal="false" destroy-on-close custom-class="apple-dialog">
      <el-form label-width="100px" style="padding-right: 40px" :model="form" ref="formRef">
        <el-form-item label="轮播图">
          ...
          >
            <img v-if="form.img" :src="form.img" class="uploaded-carousel">
            <div v-else class="upload-placeholder carousel-placeholder">
  ...
  methods: {
    ...
    del(id) {   // 单个删除
      this.$confirm('您确定删除吗？', '确认删除', {
        ...
        this.$request.delete('/carousel/delete/' + id).then(res => {
    ...
    delBatch() {   // 批量删除
      ...
      this.$confirm('您确定批量删除这些数据吗？', '确认删除', {
        ...
        this.$request.delete('/carousel/delete/batch', {data: this.ids}).then(res => {
          if (res.code === '200') {   // 表示操作成功
            this.$message.success('操作成功')
            this.load(1)
          } else {
