...
export default {
  ...
  methods: {
    changeStatus(row, status) {
      ...
      data.status = status
      this.$request.put('/activity/update', data).then(res => {
        if (res.code === '200') {
...
.apple-input >>> .el-input__inner:focus { background-color: #fff; border-color: var(--apple-blue); box-shadow: 0 0 0 3px var(--apple-blue-light); }
.apple-btn { height: 36px; padding: 0 20px; border-radius: 18px; font-weight: 500; border: none; transition: var(--apple-transition); }
.apple-btn.plain { background-color: var(--apple-bg-color); color: var(--apple-text-color); }
.table-img { width: 44px; height: 44px; border-radius: 12px; box-shadow: var(--apple-shadow-sm); object-fit: cover; }
