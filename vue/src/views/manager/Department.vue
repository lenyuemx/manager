<template>
  <div class="manager-page-container">
    <div class="search card glass-effect">
      <el-input prefix-icon="el-icon-search" placeholder="请输入社团名称" class="apple-input" v-model="name" clearable @clear="load(1)"></el-input>
    ...
    <div class="table card glass-effect">
      <el-table :data="tableData" class="apple-table" @selection-change="handleSelectionChange" :header-cell-style="{background:'#fcfcfc',color:'#86868b',fontWeight:'600'}">
        ...
        <el-table-column prop="description" label="社团介绍" show-overflow-tooltip>
          <template v-slot="scope">
            <el-button type="text" class="apple-text-btn" @click="viewEditor(scope.row.description)">查看介绍</el-button>
          </template>
        </el-table-column>
    ...
    <el-dialog title="社团信息" :visible.sync="fromVisible" width="600px" :close-on-click-modal="false" destroy-on-close @close="cancel" custom-class="apple-dialog">
      <el-form label-width="100px" style="padding-right: 40px" :model="form" :rules="rules" ref="formRef">
        ...
        <el-form-item prop="name" label="社团名称">
          <el-input v-model="form.name" autocomplete="off" class="apple-input" placeholder="请输入社团名称"></el-input>
        </el-form-item>
        <el-form-item prop="userId" label="社长姓名">
          <el-select v-model="form.userId" placeholder="请选择社长" style="width: 100%" class="apple-input">
            <el-option v-for="item in headerData" :key="item.id" :label="item.name" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item prop="teacherId" label="指导老师">
          <el-select v-model="form.teacherId" placeholder="请选择指导老师" style="width: 100%" class="apple-input">
    ...
    <el-dialog title="社团介绍" :visible.sync="editorVisible" width="60%" custom-class="apple-dialog">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        userId: [
          {required: true, message: '请选择社团社长', trigger: 'change'},
        ],
        teacherId: [
  ...
  methods: {
    ...
    loadTeachers(){
      this.$request.get('/user/selectAll?role=TEACHER').then(res =>{
        if(res.code === '200'){
