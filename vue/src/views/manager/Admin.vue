<template>
  <div class="manager-page-container">
    ...
    <div class="table card glass-effect">
      <el-table :data="tableData" class="apple-table" @selection-change="handleSelectionChange" :header-cell-style="{background:'#fcfcfc',color:'#86868b',fontWeight:'600'}">
        ...
        <el-table-column prop="role" label="角色">
          <template v-slot="scope">
            <span class="role-badge" :class="scope.row.role === 'ADMIN' ? 'admin' : 'user'">{{ scope.row.role === 'ADMIN' ? '管理员' : '普通用户' }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog title="管理员信息" :visible.sync="fromVisible" width="500px" :close-on-click-modal="false" destroy-on-close custom-class="apple-dialog">
